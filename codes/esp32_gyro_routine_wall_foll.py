# this contains starting section gyro follow straight and reaches front distance < 20 stop and make turn
# decide the clockwise or ccw to follow the outer line wall following, no lapse counting
...
ROBOT_SPEED = 160            # Baseline track speed sent to ESP32 (0-255)
...
# NEW: Outer Wall-Following Clearance Target (20 cm)
        ...
        with LidarScanner() as scanner:
            while True:
                ...
                if active_run_state == STATE_CORNERING:
                    ...
                    # Target threshold check (90° rotation profile complete)
                    if abs(yaw_delta) >= TURN_TARGET_DEGREES: 
                        ...
                        # NEW: Increment turn count tracker upon successful corner exit
                        turn_count += 1
                        print(f"[STATUS] Turn Completed! Absolute Race Turn Count: {turn_count}")
                ...
                if avg_front_distance < FRONT_TURN_TRIGGER_MM:
                    ...
                    if avg_left_space < avg_right_space:
                        ...
                        # NEW: Set directional flag strictly on the first turn
                        if turn_count == 0:
                ...
                # ====================================================
                # STATE 3: INTERTIAL NAVIGATION & WALL-FOLLOW ARBITRATION
                # ====================================================
                # OPTIMIZATION: Check if we have completed at least 1 turn and locked down track direction
                if turn_count >= 1 and clockwise_layout is not None:
                    ...
                    if clockwise_layout:
                        ...
                        if left_follow_pts:
                            ...
                            # Error > 0 means too far right (steer left). Error < 0 means too close left (steer right).
                            ...
                            # Instant safety fallback to pure gyro lock if left wall values momentarily clip out
                        ...
                        if right_follow_pts:
                            ...
                            # Error > 0 means too far left (steer right). Error < 0 means too close right (steer left).
                            ...
                            # Instant safety fallback to pure gyro lock if right wall values momentarily clip out
                            heading_error = 0.0 - current_yaw
                            pid_output = gyro_straight_pid.update(heading_error)
                            target_servo_angle = SERVO_CENTER_ANGLE - pid_output
