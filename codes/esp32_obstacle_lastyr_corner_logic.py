...
######corner logic using gyro sensor
...
def robot_control_loop():
    ...
    try:
        while not global_shutdown_event.is_set():
            ...
            print("vision_angle: ", vision_angle)
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                # PHASE 3: Active arc tracking loop
                    ...
                    display_text = f"[CORNER] Phase 3: Pivot | Yaw: {current_yaw:+.1f}°"
                    print(f"[CORNER EXECUTION] Phase 3: Active Pivot | Yaw Progress: {current_yaw:+.1f}°")
                    
                    turn_completed = False
                    if turn_direction == "RIGHT" and current_yaw <= -30.0:   
                        turn_completed = True
                    elif turn_direction == "LEFT" and current_yaw >= 40.0:  
                        turn_completed = True
                ...
                # PHASE 4: Drive backward for 2 seconds to expand camera sight lines
                    ...
                    display_text = f"[CORNER] Phase 4: Backup | Time: {elapsed_back:.1f}s"
                ...
                # Render overlays during the cornering sub-states
            ...
            if scan_data:
                print(f"[CORNER CHECK] Front Field: {avg_front_baseline:.1f}mm | Left Field: {avg_left:.1f}mm | Right Field: {avg_right:.1f}mm")

            is_corner_signature = False
            if (avg_front_baseline <= 1000.0) and ((avg_left < 900.0 and avg_right > 2000.0) or (avg_right < 900.0 and avg_left > 2000.0)):
                is_corner_signature = True

            if is_corner_signature:
                print(f"\n==========================================================")
                print(f"[SIGNATURE MATCHED] Corner Approach Signature Detected!")
                print(f"Front Avg: {avg_front_baseline:.1f}mm | Left Side: {avg_left:.1f}mm | Right Side: {avg_right:.1f}mm")
            ...
            # Standard non-corner state overlay rendering and buffer flush
            if DEBUG_UI_OVERLAYS:
                loop_duration = time.monotonic() - loop_start_time
                fps = 1.0 / loop_duration if loop_duration > 0 else 0
                cv2.putText(processed_frame, display_text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        ...
        # Crucial 4-space indentation fix applied below
...
if __name__ == '__main__':
    ...
    # /home/pi8/wrofe2025/env_test/bin/python /home/pi8/wrofe2025/esp32_obstacle_lastyr_corner_logic.py