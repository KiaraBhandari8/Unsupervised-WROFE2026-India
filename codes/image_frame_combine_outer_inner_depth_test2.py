...
# --- PID GAINS ---
KP_STEERING = 0.36
KD_STEERING = 0.1   # NEW: derivative gain for obstacle-avoidance steering. Start small and tune up.
...
OBSTACLE_MEMORY_TIME = 0.05  # seconds to continue avoiding after obstacle disappears

_last_obstacle_seen_time = None
_last_logic_label = None
_last_steering_angle = 0.0
...
def process_frame_for_steering(frame, use_outer_roi_and_bottom_point=False):
    ...
    global _last_obstacle_seen_time
    global _last_logic_label
    global _last_steering_angle
    ...
    if chosen_contour is not None:
        ...
        _last_obstacle_seen_time = time.time()
        _last_logic_label = logic_label
        _last_steering_angle = steering_angle
    else:
    ...
    if logic_label == "none":
        ...
        if (
            _last_obstacle_seen_time is not None
            and now - _last_obstacle_seen_time < OBSTACLE_MEMORY_TIME
        ):
            steering_angle = _last_steering_angle
            logic_label = _last_logic_label + "_memory"
