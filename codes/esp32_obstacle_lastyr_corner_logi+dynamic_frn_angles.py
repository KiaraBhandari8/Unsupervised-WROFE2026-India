#in this code, includes corner logic + dynamic front angles compensated from the deviation of gyro angles 
...
def get_compensated_front_distance(scan_data, current_yaw):
    ...
    # 1. Swivel center index pointer matching active body drift
    yaw_offset = int(round(current_yaw))
    dynamic_angles = range(-FRONT_SCAN_ANGLE_DEG + yaw_offset, FRONT_SCAN_ANGLE_DEG + yaw_offset + 1)

    # 2. Accumulate points and flatten vectors using the cosine perpendicular formula
    compensated_points = []
    ...
    try:
        while not global_shutdown_event.is_set():
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                # PHASE 2: Approach wall using the target compensated logic helper
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    # IMPLEMENTATION: Applied the pure gyro lookcone stabilization function here
                    avg_front = get_compensated_front_distance(scan_data, current_yaw)
                    
                    print(f"[CORNER EXECUTION] Phase 2: Approaching wall | Compensated Front: {avg_front:.1f}mm / 450.0mm")
            ...
            # ====================================================
            # IMPLEMENTATION: Inserted the pure gyro-compensated lookcone calculator function here
            avg_front = get_compensated_front_distance(scan_data, current_yaw)
            ...
            if scan_data:
                print(f"[CORNER CHECK] Compensated Front: {avg_front:.1f}mm | Left: {avg_left:.1f}mm | Right: {avg_right:.1f}mm")

            is_corner_signature = False
            if (avg_front <= 1000.0) and ((avg_left < 900.0 and avg_right > 600.0) or (avg_right < 900.0 and avg_left > 1400.0)):
