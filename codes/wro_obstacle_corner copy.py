...
# --- OBSTACLE AVOIDANCE (ESP32-style reactive proportional steering) ---
# No committed turn/realign/recenter state machine: every frame that a pillar
# is visible on the track, steer away from it by an amount proportional to how
# far it has drifted from the frame centre. RED -> pass on the RIGHT (steer
# right), GREEN -> pass on the LEFT (steer left). This mirrors the original
# ESP32 obstacle-round firmware's PRIORITY LEVEL 4 vision handler, which drove
# `target_servo_angle = SERVO_CENTER_ANGLE -(+) vision_angle * STEERING_GAIN`
# off a lane-steering vision angle. That angle came from an outer/inner-ROI
# module this codebase doesn't have, so it's reconstructed here from the LAB
# pillar's horizontal offset (cx) converted to a pseudo-degree value over the
# camera's horizontal field of view -- same formula and gains, different angle
# source.
CAMERA_HALF_FOV_DEG = 50.0       
STEERING_GAIN_RED = 0.65         # Boosted from 0.14 to turn sharper right
STEERING_GAIN_GREEN = 0.75       # Boosted from 0.10 to turn sharper left
RED_CLEARANCE_OFFSET = 8         
VISION_AVOID_MIN_WIDTH = 30    # LAB-frame px width before a pillar engages avoidance
...
def vision_pillar_avoidance(dets, lab_w):
    """Reactive proportional pillar avoidance (ESP32 obstacle-round style).

    No turn/realign/recenter state machine -- every frame that a red or green
    pillar is visible on the track, steer away from it by an amount
    proportional to how far its centre has drifted from the middle of the
    frame. RED -> pass on the RIGHT, GREEN -> pass on the LEFT. As soon as the
    pillar leaves view (or drifts behind the robot) this simply stops firing
    and priority falls through to whatever's next (LiDAR side check / wall
    following) -- there's no committed maneuver to unwind.
    ...
    # this is a far more reliable "nearest" signal than raw blob area.
    target = max(objs, key=lambda d: (d['bbox'][3], d['area']))
    print(
        f"class={target['class']}, "
        f"cx={target['cx']}, "
        f"bbox={target['bbox']}"
    )
    if target['width'] < VISION_AVOID_MIN_WIDTH:
        return False, None, "none"

    color = target['class']
    cx = target['cx']

    # Pillar's horizontal offset from frame centre, normalised to +/-1 and
    # scaled to a pseudo-degree "vision angle" over the camera's horizontal FOV.
    normalized_offset = (cx - lab_w / 2.0) / (lab_w / 2.0)
    vision_angle = normalized_offset * CAMERA_HALF_FOV_DEG

    if color == 'red':
        servo_adjust = abs(vision_angle) * STEERING_GAIN_RED
        target_servo_angle = SERVO_CENTER_ANGLE + servo_adjust + RED_CLEARANCE_OFFSET
        label = "red_obstacle"
    else:
        servo_adjust = abs(vision_angle) * STEERING_GAIN_GREEN
        target_servo_angle = SERVO_CENTER_ANGLE - servo_adjust
        
        # Absolute safeguard: If it is green, force the steering angle to be less than 90
        if target_servo_angle > 88:
            target_servo_angle = 82

        label = "green_obstacle"

    return True, target_servo_angle, label
...
def _primary_obstacle(objs):
    """Mirror vision_pillar_avoidance's 'nearest' pick for DISPLAY only.
...
# The gyro yaw stream (current_yaw / gyro_ok, read off the ESP32's "YAW:..."
# line by gyro_acquisition_thread_func) is kept for the corner state
# machine's optional front-distance compensation below -- it's no longer
# used for post-avoidance lane recentering since vision_pillar_avoidance()
# is a stateless, per-frame reactive handler with nothing to recenter after.
...
# --- CORNER (90-DEGREE TURN) STATE MACHINE ---
# Recognises an approaching corner from LiDAR asymmetry (front closing in,
# one side tight, the other open) and then drives a deterministic, timed
# pivot: brake -> hard-lock the steering -> hold the pivot for a calibrated
# duration -> back off the corner wall -> hand back to LiDAR wall following.
# The pivot/backward phases are open-loop (time-based, not gyro-based) so the
# manoeuvre still works when the ESP32 isn't streaming yaw (gyro_ok is False).
CORNER_FRONT_SCAN_DEG = 15          # +/- degrees sampled for the front baseline
CORNER_SIGNATURE_FRONT_MM = 1300.0  # front must be closing to at least this range
CORNER_SIGNATURE_TIGHT_MM = 950.0   # the wall-side reading must be tighter than this
CORNER_SIGNATURE_OPEN_MM = 1400.0   # the open-side reading must be at least this
CORNER_APPROACH_STOP_MM = 550.0     # keep creeping toward the wall until this close
CORNER_PIVOT_DURATION_SEC = 1.5    # time to hold the hard-locked pivot
CORNER_BACKWARD_DURATION_SEC = 1.5  # time to back away from the corner wall
CORNER_BRAKE_DELAY_SEC = 0.25       # brief brake pause between phases (kills back-EMF)
CORNER_HALT_DELAY_SEC = 1.0         # full stop while committing to a turn direction
CORNER_COOLDOWN_SEC = 2.5           # ignore new corner signatures right after finishing one
CORNER_PIVOT_SPEED = ROBOT_CRUISE_SPEED     # forward speed while pivoting
CORNER_BACK_SPEED = ROBOT_MANEUVER_SPEED    # reverse speed while backing off
...
class RobotState:
    ...
    # Corner (90-degree turn) sub-states, executed in sequence by
    # manage_corner_turn(): approach the wall, pivot open-loop, back off.
...
# --- Corner (90-degree turn) state -------------------------------------
# Lives at module level so it persists cleanly across control-loop
# iterations without threading extra arguments everywhere.
corner_turn_direction = None      # "LEFT" or "RIGHT" once a corner is committed to
corner_pivot_start_time = 0.0
corner_backward_start_time = 0.0
corner_cooldown_until = 0.0


def _corner_side_baselines(scan_data, current_yaw=None):
    """Front / left / right average LiDAR ranges used to recognise a corner.

    Front uses a wider +/- CORNER_FRONT_SCAN_DEG cone than the pillar-avoidance
    front reading so an approaching wall is caught a little earlier; left/right
    reuse the same wall-following angle bands used elsewhere in this file.
    When a gyro heading is available the front cone is shifted by the current
    yaw and range-corrected by cos(yaw), so the reading stays a true "distance
    straight ahead" even while the robot is a little off-heading."""
    if not scan_data:
        return 2000.0, 2000.0, 2000.0

    yaw_offset = int(round(current_yaw)) if current_yaw is not None else 0
    front_lo = -CORNER_FRONT_SCAN_DEG + yaw_offset
    front_hi = CORNER_FRONT_SCAN_DEG + yaw_offset

    front_pts = [d for a, d in scan_data.items() if front_lo <= a <= front_hi and d > 0]
    left_pts = [d for a, d in scan_data.items() if -90 <= a <= -40 and d > 0]
    right_pts = [d for a, d in scan_data.items() if 40 <= a <= 90 and d > 0]

    avg_front = sum(front_pts) / len(front_pts) if front_pts else 2000.0
    if current_yaw is not None and front_pts:
        avg_front *= abs(np.cos(np.radians(current_yaw)))
...
def _is_corner_signature(avg_front, avg_left, avg_right):
    """True when the LiDAR profile matches an approaching 90-degree corner:
    the front is closing in AND exactly one side is noticeably tighter than
    the other (that tight side is the wall the track is about to turn into,
    the open side is the direction the track bends toward)."""
    if avg_front > CORNER_SIGNATURE_FRONT_MM:
        return False
    left_wall = avg_left < CORNER_SIGNATURE_TIGHT_MM and avg_right > CORNER_SIGNATURE_OPEN_MM
    right_wall = avg_right < CORNER_SIGNATURE_TIGHT_MM and avg_left > CORNER_SIGNATURE_OPEN_MM
    print( f"[CORNER DEBUG] " f"Front={avg_front:.0f} " f"Left={avg_left:.0f} " f"Right={avg_right:.0f} " f"LeftWall={left_wall} " f"RightWall={right_wall}" )
    return left_wall or right_wall


def manage_corner_turn(scan_data, current_yaw=None, lidar_pid=None):
    """Stateful 90-degree corner handler.

    Returns (engaged, servo_angle, speed, label):
      - engaged=True means the corner state machine owns this control-loop
        iteration; the caller should apply (servo_angle, speed) and skip the
        rest of the priority ladder (obstacle avoidance / wall following).
      - engaged=False means no corner is being handled right now and the
        caller should fall through to its normal arbitration.

    State lives in module globals (corner_turn_direction, corner_pivot_start_time,
    corner_backward_start_time, corner_cooldown_until, turn_counter) so it
    persists across iterations the same way the rest of this file's stateful
    handlers do. `lidar_pid` is optional -- if given, its PID state is reset
    once the turn completes so wall-following doesn't inherit stale
    integral/derivative terms from before the corner.
    """
    global current_robot_state, corner_turn_direction
    global corner_pivot_start_time, corner_backward_start_time, corner_cooldown_until
    global turn_counter, straight_detected_time

    now = time.monotonic()

    # ---- PHASE 2: creep toward the corner wall until close enough to pivot ----
    if current_robot_state == RobotState.CORNER_APPROACH_WALL:
        avg_front, _, _ = _corner_side_baselines(scan_data, current_yaw)
        if avg_front < CORNER_APPROACH_STOP_MM:
            stop_robot()
            time.sleep(CORNER_BRAKE_DELAY_SEC)
            hard_angle = SERVO_MAX if corner_turn_direction == "RIGHT" else SERVO_MIN
            cmd(hard_angle, 0)
            time.sleep(0.15)
            corner_pivot_start_time = now
            current_robot_state = RobotState.CORNER_ACTIVE_PIVOT
            print(f"[CORNER] Approach limit reached ({avg_front:.0f}mm). Pivoting {corner_turn_direction}...")
            return True, hard_angle, CORNER_PIVOT_SPEED, f"[CORNER] Pivot start ({corner_turn_direction})"
        return True, SERVO_CENTER_ANGLE, ROBOT_CRUISE_SPEED, f"[CORNER] Approach | Front:{avg_front:.0f}mm"

    # ---- PHASE 3: open-loop timed pivot, wheels hard-locked ----
    if current_robot_state == RobotState.CORNER_ACTIVE_PIVOT:
        elapsed = now - corner_pivot_start_time
        hard_angle = SERVO_MAX if corner_turn_direction == "RIGHT" else SERVO_MIN
        if elapsed >= CORNER_PIVOT_DURATION_SEC:
            stop_robot()
            time.sleep(CORNER_BRAKE_DELAY_SEC)
            corner_backward_start_time = now
            current_robot_state = RobotState.CORNER_ALIGN_BACKWARD
            print("[CORNER] Pivot window complete. Backing off the wall...")
            return True, SERVO_CENTER_ANGLE, -CORNER_BACK_SPEED, "[CORNER] Backing off"
        return True, hard_angle, CORNER_PIVOT_SPEED, f"[CORNER] Pivot {elapsed:.2f}/{CORNER_PIVOT_DURATION_SEC}s"

    # ---- PHASE 4: back away from the wall just turned into ----
    if current_robot_state == RobotState.CORNER_ALIGN_BACKWARD:
        elapsed = now - corner_backward_start_time
        if elapsed >= CORNER_BACKWARD_DURATION_SEC:
            stop_robot()
            time.sleep(0.3)
            turn_counter += 1
            corner_cooldown_until = time.time() + CORNER_COOLDOWN_SEC
            corner_turn_direction = None
            straight_detected_time = 0.0
            if lidar_pid is not None and hasattr(lidar_pid, "reset"):
                lidar_pid.reset()
            current_robot_state = RobotState.LIDAR_WALL_FOLLOWING
            print(f"[CORNER] Turn #{turn_counter} complete. Resuming wall following.")
            return False, SERVO_CENTER_ANGLE, 0, f"[CORNER] Turn #{turn_counter} complete"
        return True, SERVO_CENTER_ANGLE, -CORNER_BACK_SPEED, f"[CORNER] Backing {elapsed:.2f}/{CORNER_BACKWARD_DURATION_SEC}s"

    # ---- Not currently cornering: watch for a new corner signature ----
    if turn_counter >= max_turn_count:
        return False, None, None, None
    if time.time() < corner_cooldown_until:
        return False, None, None, None

    avg_front, avg_left, avg_right = _corner_side_baselines(scan_data, current_yaw)
    if not _is_corner_signature(avg_front, avg_left, avg_right):
        return False, None, None, None

    # Commit to the turn: brief full stop, pick a direction from whichever
    # side is open (that's the way the track bends), then creep toward the
    # near wall before pivoting.
    stop_robot()
    time.sleep(CORNER_HALT_DELAY_SEC)
    corner_turn_direction = "RIGHT" if avg_left < avg_right else "LEFT"
    print(f"[CORNER] Signature detected | Front:{avg_front:.0f} Left:{avg_left:.0f} "
          f"Right:{avg_right:.0f} -> turning {corner_turn_direction}")
    current_robot_state = RobotState.CORNER_APPROACH_WALL
    return True, SERVO_CENTER_ANGLE, ROBOT_CRUISE_SPEED, f"[CORNER] Signature -> {corner_turn_direction}"
...
def robot_control_loop(shutdown_event):
    ...
    try:
        ...
        while not shutdown_event.is_set():
            ...
            det_method = "LAB"
            # Reactive per-frame avoidance (ESP32 style) -- no committed
            # turn/realign/recenter sequence, just steer away proportionally
            # while a pillar is in view.
            avoid_engaged, avoid_angle_val, logic_label = vision_pillar_avoidance(
                detections, lab.shape[1])
            ...
            # Corner FSM gets a look every loop UNLESS an actual imminent
            # collision is in progress -- that stays the one thing allowed to
            # override a corner pivot outright. Skipping the call (rather than
            # just ignoring its result) also avoids taking the state machine's
            # blocking brake/halt pauses during a genuine emergency.
            corner_engaged, corner_angle, corner_speed, corner_label = (False, None, None, None)
            if escape_direction is None:
                corner_engaged, corner_angle, corner_speed, corner_label = manage_corner_turn(
                    scan_data, current_yaw=current_yaw, lidar_pid=lidar_pid)
            ...
            # PRIORITY 1: CORNER TURN (deterministic 90-degree pivot).
            # A corner in progress -- or a freshly detected one -- outranks
            # pillar avoidance and LiDAR wall following; these tracks don't put
            # pillars in corners, so once the state machine commits to a pivot
            # it should run to completion rather than get pre-empted by a
            # stray colour blob or side-wall reading. Only imminent collision
            # (PRIORITY 0) is allowed to interrupt it.
            elif corner_engaged:
                robot_speed_current = corner_speed
                target_servo_angle = corner_angle
                display_text = f"MODE: {corner_label}"
                if time.time() - print_timer >= 0.5:
                    print_timer = time.time()
                    print(corner_label)

            # PRIORITY 2: SIDE OBSTACLE (LIDAR proximity override). Matches
            # the ESP32 firmware's ordering: a wall crowding a side sensor
            # outranks pillar avoidance, since steering further into a pillar
            # pass while also brushing a side wall is worse than briefly
            # deferring the pillar correction.
            ...
            # PRIORITY 3: CAMERA OBSTACLE (LAB colour), reactive proportional
            # steering (ESP32 style -- see vision_pillar_avoidance()).
            elif avoid_engaged:
                robot_speed_current = ROBOT_MANEUVER_SPEED
            ...
            # PRIORITY 4: LIDAR WALL FOLLOWING
            ...
            if time.time() - print_timer >= 0.5:
                print_timer = time.time()
                print(f"[{loop_counter}] {current_robot_state} | Angle:{final_angle} Speed:{robot_speed_current:.2f} FPS:{int(fps)} Det:{det_method}({len(detections)}) {logic_label} Turns:{turn_counter}/{max_turn_count}")
            ...
            if STREAM_VIDEO:
                ...
                if DEBUG_UI_OVERLAYS:
                    ...
                    cv2.putText(processed_frame, f"Turns: {turn_counter}/{max_turn_count}", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
...
def get_lidar():
    with lidar_data_lock:
        scan = latest_lidar_data.copy()

    def avg_distance(start_angle, end_angle):
        values = [
            d for a, d in scan.items()
            if start_angle <= a <= end_angle and d > 0
        ]
        return sum(values) / len(values) if values else None

    return jsonify({
        "front": avg_distance(-10, 10),
        "left": avg_distance(-90, -11),
        "right": avg_distance(11, 90)
    })
