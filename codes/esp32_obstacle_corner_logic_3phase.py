...
FRONT_TURN_TRIGGER_MM = 400.0  # Strict 20cm front trigger boundary
...
#value for avg right is 1900 initially
            ...
            # ====================================================
                ...
                if abs(vision_angle) > 35 or swerve_elapsed > SWERVE_SAFETY_TIMEOUT:
                    if swerve_elapsed > SWERVE_SAFETY_TIMEOUT and abs(vision_angle) <= 35:
                        print(f"[{current_robot_state}] [EVADE] WARNING: SWERVE safety timeout hit (vision_angle only {vision_angle:.1f}°). Forcing exit to CLEARING.")
                    ...
                    if CLOCKWISE_WALL_FOLLOWING:
                        ...
                        if left_follow_pts:
                            ...
                            if avg_left_wall > WALL_LOSS_THRESHOLD_MM:
                                ...
                                target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                                display_text = f"MODE: Follow Outer Left | Err: {wall_error:.0f}mm | Yaw: {current_yaw:+.1f}°"
                        else:
                            heading_error = 0.0 - current_yaw
                            pid_output = gyro_straight_pid.update(heading_error)
                            target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                        ...
                        if right_follow_pts:
                            ...
                            if avg_right_wall > WALL_LOSS_THRESHOLD_MM:
                                ...
                                pid_output = wall_follow_pid.update(wall_error)
                                target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                                display_text = f"MODE: Follow Outer Right | Err: {wall_error:.0f}mm | Yaw: {current_yaw:+.1f}°"
                        else:
                            heading_error = 0.0 - current_yaw
                            pid_output = gyro_straight_pid.update(heading_error)
                            target_servo_angle = SERVO_CENTER_ANGLE - pid_output
