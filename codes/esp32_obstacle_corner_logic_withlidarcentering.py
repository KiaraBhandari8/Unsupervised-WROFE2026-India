...
WALL_LOSS_THRESHOLD_MM = 750.0  # Open pocket validation limit; must exceed WALL_FOLLOW_TARGET_MM (500) or it false-triggers at normal following distance
...
# Hybrid wall-centering hysteresis thresholds
BALANCE_ENTER_MM = 250   # enter centering mode when |L-R| drops below this
BALANCE_EXIT_MM = 700    # exit centering mode when |L-R| rises above this
...
def robot_control_loop():
    ...
    center_pid = PIDController(Kp=0.2, Ki=0.001, Kd=0.03, setpoint=0)

    current_robot_state = RobotState.PURE_GYRO_START
    turn_count = 0
    baseline_start_yaw = 0.0
    turn_direction = None
    in_centering_mode = True
        ...
        while not global_shutdown_event.is_set():
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    ...
                    print(f"[CORNER EXECUTION] Phase 2: Approaching wall | Front: {avg_front_baseline:.1f}mm / 390.0mm")
                    
                    if avg_front_baseline < 390.0:
                        print("[CORNER EXECUTION] Approach limit reached. Applying hard brake...")
                        send_esp_packet(esp_ser, SERVO_CENTER_ANGLE, 0)
                        ...
                        print(f"[CORNER EXECUTION] Locking wheels to extreme pivot angle: {final_servo}°")
                ...
                # previously-working version: RIGHT pivots go negative, LEFT go positive.
                    ...
                    if pivot_complete or pivot_timed_out:
                        if pivot_timed_out and not pivot_complete:
                            print(f"[CORNER EXECUTION] WARNING: Pivot safety timeout hit before target yaw reached (only {yaw_delta_signed:+.1f}° / target {target_degrees}°). Check gyro data.")
                        print(f"[CORNER EXECUTION] Pivot complete ({yaw_delta_signed:+.1f}°). Hard braking chassis...")
                        send_esp_packet(esp_ser, SERVO_CENTER_ANGLE, 0)
                        time.sleep(CORNER_BRAKE_DELAY)  # Stop counter-momentum before shifting transmission

                        print("[CORNER EXECUTION] Resetting chassis orientation before backup...")
                        esp_ser.write(b"RST_YAW\n")
                        esp_ser.flush()
                        time.sleep(0.1)
                        current_yaw = 0.0  # matches local state to the reset we just sent

                        print("[CORNER EXECUTION] Wheels centered. Actuating Reverse Profile...")
                ...
                # PHASE 4: Drive backward away from corner obstacle
                    ...
                    print(f"[CORNER EXECUTION] Phase 4: Reversing | Elapsed: {elapsed_back:.2f}s")
                    ...
                    if elapsed_back >= CORNER_BACKWARD_DURATION:
                        ...
                        print("[CORNER EXECUTION] Cleaning up spatial registers before resuming wall-follow...")
                        esp_ser.write(b"RST_YAW\n")
                        esp_ser.flush()
                        time.sleep(0.1)
                        current_yaw = 0.0
            ...
            if scan_data:
                print(f"[CORNER CHECK] Front Field: {avg_front_baseline:.1f}mm | Left: {avg_left:.1f}mm | Right: {avg_right:.1f}mm")
#value for avg right is 1900 initially

            is_corner_signature = False
            if (avg_front_baseline <= 1000.0) and ((avg_left < 950.0 and avg_right > 1900.0) or (avg_right < 900.0 and avg_left > 1800.0)):
            ...
            if avoid_state == "CLEARING":
                ...
                if now < recovery_end_time:
                    ...
                    print("[EVADE] Recovery complete. Returning to baseline driving.")
                    avoid_state = "IDLE"
            ...
            # ====================================================
                ...
                display_text = f"MODE: Swerve {avoid_direction} | Steer: {int(target_servo_angle)}°"

                # Watchdog: pillar sliding to frame edge -> commit to blind flank pass
                if abs(vision_angle) > 35:
            ...
            # ====================================================
                ...
                avg_left_wall = sum(left_follow_pts) / len(left_follow_pts) if left_follow_pts else None
                avg_right_wall = sum(right_follow_pts) / len(right_follow_pts) if right_follow_pts else None
                both_walls_present = avg_left_wall is not None and avg_right_wall is not None
                any_wall_present = avg_left_wall is not None or avg_right_wall is not None

                if both_walls_present:
                    diff = abs(avg_left_wall - avg_right_wall)
                    if in_centering_mode and diff > BALANCE_EXIT_MM:
                        in_centering_mode = False
                    elif not in_centering_mode and diff < BALANCE_ENTER_MM:
                        in_centering_mode = True

                    print(f"[BALANCE] L:{avg_left_wall:.0f} R:{avg_right_wall:.0f} Diff:{diff:.0f}mm Mode:{'CENTER' if in_centering_mode else 'SINGLE'}")

                current_robot_state = RobotState.LIDAR_WALL_FOLLOWING

                if in_centering_mode and both_walls_present:
                    lateral_error = avg_right_wall - avg_left_wall
                    pid_output = center_pid.update(lateral_error)
                    target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                    display_text = f"MODE: Center Walls | Diff: {diff:.0f}mm"

                elif CLOCKWISE_WALL_FOLLOWING:
                    if avg_left_wall is not None:
