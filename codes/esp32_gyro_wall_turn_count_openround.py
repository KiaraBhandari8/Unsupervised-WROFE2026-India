#code contains wall follow gyro straight and turn count check to stop the robot after it reaches the count
...
# GYRO CORNERING TUNING
TURN_TARGET_DEGREES = 80.0  # Accounts for kinetic momentum overshoot braking

# RACE MANAGEMENT PARAMETERS
TOTAL_RACE_TURNS = 12        # 12 turns equals exactly 3 completed laps
LAP_FINISH_DRIVE_TIME = 4.0  # Time in seconds to drive straight over the finish line
...
# Outer Wall-Following Clearance Target (20 cm)
WALL_FOLLOW_TARGET_MM = 200.0
WALL_LOSS_THRESHOLD_MM = 350.0  # Gating threshold to detect an opening corner pocket
...
print("[SYSTEM] Initializing State-Locking Outer Wall Following Engine...")
time.sleep(2)
...
def main():
    ...
    # TURN AND LAP TRACKING REGISTERS
    turn_count = 0             # Tracking the absolute number of completed turns
    clockwise_layout = None    # Saved layout state: True = CW (Follow Left), False = CCW (Follow Right)
    ...
    # Core stabilization control loop profiles
    ...
    try:
            ...
            while True:
                ...
                if active_run_state == STATE_CORNERING:
                    ...
                    if abs(yaw_delta) >= TURN_TARGET_DEGREES: 
                        print("\n==========================================================")
                        print(f"[TARGET CONFIRMED] Turn met at complete delta: {yaw_delta:.1f}°")
                        print("==========================================================")
                        
                        # Increment turn counter immediately upon completing the turn arc
                        turn_count += 1
                        lap_count = turn_count // 4
                        print(f"[RACE LOGGER] Turn completed! Total Turns: {turn_count} | Estimated Laps: {lap_count}")
                        
                        # INTEGRATED LAP TERMINATION GATE
                        if turn_count >= TOTAL_RACE_TURNS:
                            print("\n==========================================================")
                            print(f"[MATCH COMPLETE] {TOTAL_RACE_TURNS} Turns Logged Successfully!")
                            print(f"[ACTION] Locking wheels straight. Crossing finish line for {LAP_FINISH_DRIVE_TIME}s...")
                            print("==========================================================")
                            
                            # Force the chassis straight while holding baseline propulsion speed
                            finish_packet = f"STR:{SERVO_CENTER_ANGLE},SPD:{ROBOT_SPEED}\n"
                            esp_ser.write(finish_packet.encode('utf-8'))
                            
                            # Time delay window block to let the robot cruise over the final line mats
                            time.sleep(LAP_FINISH_DRIVE_TIME)
                            
                            # Break out of the execution wire loop to shut down
                            print("\n[ACTION] Finish line passed. Issuing permanent brake shutdown.")
                            break
                        
                        # Standard corner-exit cleanup routine (Only runs if under 12 turns)
                        print("[ACTION] Halting motors and centering wheels back to 95° at a stop...")
                        esp_ser.write(f"STR:{SERVO_CENTER_ANGLE},SPD:0\n".encode('utf-8'))
                        time.sleep(0.4)  # Let kinetic slide settle down completely
                        
                        print("[ACTION] Dispatching register clear -> Resetting Gyro Yaw to 0°...")
                        esp_ser.write("RST_YAW\n".encode('utf-8'))
                        time.sleep(0.2)  # Settle time for firmware register updates
                        
                        current_yaw = 0.0
                        gyro_straight_pid.prev_error = 0 
                        wall_follow_pid.prev_error = 0
                        
                        print("[STATE TRANSITION] Path cleared. Resuming forward tracking layout.\n")
                        active_run_state = STATE_LANE_FOLLOWING
                        turn_direction = None
                    else:
                ...
                if avg_front_distance < FRONT_TURN_TRIGGER_MM:
                    ...
                    if avg_left_space < avg_right_space:
                        ...
                        if turn_count == 0:
                            clockwise_layout = True
                            print("[DECISION] Turn 1 Detected: Left Space < Right Space -> CLOCKWISE (CW)")
                    else:
                        turn_direction = "LEFT"
                        final_servo_angle = SERVO_HARD_LEFT
                        if turn_count == 0:
                            clockwise_layout = False
                            print("[DECISION] Turn 1 Detected: Left Space >= Right Space -> COUNTER-CLOCKWISE (CCW)")
                ...
                # ====================================================
                # STATE 3: INERTIAL NAVIGATION & WALL-FOLLOW ARBITRATION
                # ====================================================
                if turn_count >= 1 and clockwise_layout is not None:
                    
                    if clockwise_layout:
                        # --- CLOCKWISE MODE: Follow OUTER LEFT wall (-90° to -40°) ---
                        left_follow_angles = [a for a in range(-90, -39)]
                        left_follow_pts = [scan_data[a] for a in left_follow_angles if a in scan_data and scan_data[a] > 0]
                        ...
                        if left_follow_pts:
                            ...
                            if avg_left_wall > WALL_LOSS_THRESHOLD_MM:
                                ...
                                print(f"[WALL LOST FALLBACK] Left Wall Opened ({avg_left_wall:.1f}mm). Locking Gyro Straight | Servo: {int(target_servo_angle)}°")
                            else:
                                wall_error = avg_left_wall - WALL_FOLLOW_TARGET_MM
                                pid_output = wall_follow_pid.update(wall_error)
                                target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                                print(f"[STATE_WALL_FOLLOW] CW Layout | Left Wall: {avg_left_wall:.1f}mm | Err: {wall_error:.1f}mm | Servo: {int(target_servo_angle)}°")
                        else:
                            heading_error = 0.0 - current_yaw
                            pid_output = gyro_straight_pid.update(heading_error)
                            target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                        ...
                        # --- COUNTER-CLOCKWISE MODE: Follow OUTER RIGHT wall (40° to 90°) ---
                        right_follow_angles = [a for a in range(40, 91)]
                        right_follow_pts = [scan_data[a] for a in right_follow_angles if a in scan_data and scan_data[a] > 0]
                        ...
                        if right_follow_pts:
                            ...
                            if avg_right_wall > WALL_LOSS_THRESHOLD_MM:
                                ...
                                pid_output = gyro_straight_pid.update(heading_error)
                                target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                                print(f"[WALL LOST FALLBACK] Right Wall Opened ({avg_right_wall:.1f}mm). Locking Gyro Straight | Servo: {int(target_servo_angle)}°")
                            else:
                                wall_error = WALL_FOLLOW_TARGET_MM - avg_right_wall
                                pid_output = wall_follow_pid.update(wall_error)
                                target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                                print(f"[STATE_WALL_FOLLOW] CCW Layout | Right Wall: {avg_right_wall:.1f}mm | Err: {wall_error:.1f}mm | Servo: {int(target_servo_angle)}°")
                        else:
                            heading_error = 0.0 - current_yaw
                            pid_output = gyro_straight_pid.update(heading_error)
                            target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                    ...
                    # --- INITIAL START SECTOR RUN: Pure Gyro Heading Lock (turn_count == 0) ---
                    heading_error = 0.0 - current_yaw
                    pid_output = gyro_straight_pid.update(heading_error)
                    target_servo_angle = SERVO_CENTER_ANGLE - pid_output
                    
                    print(f"[STATE_LANE_FOLLOWING] Initial Start (Pure Gyro) | Yaw: {current_yaw:.2f}° | Err: {heading_error:.2f}° | Servo: {int(target_servo_angle)}°")

                # Enforce safety constraints and deliver control packet to hardware execution layer
        ...
        # Enforce terminal brake state configuration on program termination
