...
ROBOT_CRUISE_SPEED = 165      # Operational forward driving speed sent to ESP32 (0-255)
...
CORNER_APPROACH_BRAKE_DISTANCE_MM = 460.0
...
LEFT_DISTANCE_ARC_THRESHOLD_MM = 500.0   # if avg_left > this at RIGHT-turn detection, use reverse arc instead of in-place pivot
CORNER_ARC_TRIGGER_FRONT_MM = 400.0      # tighter approach trigger distance for the arc case
        ...
        while not global_shutdown_event.is_set():
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ARC_ACTIVE_PIVOT, RobotState.CORNER_POST_PIVOT_ALIGN, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    ...
                    display_text = (
                        f"[CORNER] Phase 3.5: Post-Pivot Align | Side: {align_side.title()} | Err: {parallel_error:.1f}mm | Rear: {rear_distance if rear_distance is not None else float('nan'):.0f}mm"
                        if parallel_error is not None else
                        f"[CORNER] Phase 3.5: Post-Pivot Align | Side: {align_side.title()} | Err: N/A | Rear: {rear_distance if rear_distance is not None else float('nan'):.0f}mm"
                    )
                    ...
                    print(
                        ...
                        f"RearToF: {rear_distance} | FrontPts: {front_count} | RearPts: {rear_count} | Elapsed: {elapsed_post_pivot_align:.2f}s"
                    )

                    if tof_reached or is_aligned or hard_timeout or no_wall_timeout:
                        reason = "tof_reached" if tof_reached else "aligned" if is_aligned else "timeout" if hard_timeout else "no_wall"
                        print(f"[{current_robot_state}] [POST-PIVOT ALIGN] Exit condition reached ({reason}). Rear: {rear_distance}. Braking and finishing corner.")
                        send_esp_packet(esp_ser, SERVO_CENTER_ANGLE, 0)
                        time.sleep(CORNER_BRAKE_DELAY)

                        print(f"[{current_robot_state}] [POST-PIVOT ALIGN] Resetting spatial registers before resuming wall-follow...")
                        esp_ser.write(b"RST_YAW\n")
                        esp_ser.flush()
                        time.sleep(0.1)
                        current_yaw = 0.0
