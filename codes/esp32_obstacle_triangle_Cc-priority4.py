"""Removed the separate distance-hold PID (wall_follow_pid) used during corner approach — that state now uses the same parallel-alignment PID (alignment_pid) as normal cruising, so there's just one wall-following logic everywhere.

Changed the speed sent during corner approach from ROBOT_CRUISE_SPEED to WALL_ALIGN_CREEP_SPEED.""" 
...
WALL_FOLLOW_TARGET_MM = 600  # UNUSED now -- distance-hold wall_follow_pid logic was removed; only

                              # parallel-alignment (alignment_pid) is used everywhere now. Safe to delete.

LIDAR_SIDE_AVOIDANCE_SPEED = 160    # slower than cruise speed while actively correcting

SIDE_AVOIDANCE_POST_OBSTACLE_COOLDOWN_SEC = 1.0   # NEW: side-avoidance entry is disabled for
                                                   # this many seconds after vision obstacle
                                                   # avoidance finishes and hands off control
...
# UPDATED: single-threshold + frame-persistence-debounce side avoidance,

# ported from the standalone debug harness. Replaces the old two-zone

# WARN/PANIC design, which had an unfiltered 150-300mm middle band with

# no point-count check, and no frame-persistence -- letting a single

# noisy ray flicker avoidance on/off even in genuinely open space.
...
SIDE_MIN_TRIGGER_POINTS = 3         # rays under SIDE_TRIGGER_DISTANCE_MM required to count as "real"

SIDE_TRIGGER_CONFIRM_FRAMES = 3     # consecutive frames the point-count condition must hold to ENGAGE

SIDE_RELEASE_CONFIRM_FRAMES = 4     # consecutive CLEAR frames required to RELEASE once engaged
...
ROBOT_CRUISE_SPEED = 100      # Operational forward driving speed sent to ESP32 (0-255)
...
STEERING_GAIN_RED = 0.15      # INCREASE THIS to make the steering more aggressive for Red
...
CORNER_APPROACH_TRIGGER_MM = 500.0              # TUNED (was 550.0) -- unified 2nd-stop front trigger, both pivot & arc
...
CORNER_APPROACH_WALL_LOSS_THRESHOLD_MM = 400.0   # UNUSED now -- was only used by the removed

                                                  # distance-hold wall_follow_pid branch. Safe to delete.
...
def compute_side_avoidance_magnitude(min_dist_mm):
    ...
    """Only meaningful once a side is already ENGAGED (see debounce state in the

    main loop). Scales smoothly from SIDE_STEER_MIN_MAGNITUDE at the trigger

    edge up to SIDE_STEER_MAX_MAGNITUDE as min_dist_mm approaches 0."""
...
def robot_control_loop():
    ...
    # NEW: side-avoidance entry/exit debounce state (persists across loop iterations)

    right_side_confirm_streak = 0

    left_side_confirm_streak = 0

    right_side_clear_streak = 0

    left_side_clear_streak = 0
    ...
    post_avoid_active = False   # NEW: independent of current_robot_state
    side_avoidance_cooldown_end_time = 0.0   # NEW: side-avoidance entry disabled until this monotonic timestamp
    ...
    try:
        ...
        while not global_shutdown_event.is_set():
                ...
                if current_robot_state == RobotState.CORNER_PRE_ALIGN:
                    ...
                    if is_aligned or hard_timeout or no_wall_timeout:
                        ...
                        if parallel_error is None:
                            ...
                            final_servo_angle = int(round(np.clip(target_servo_angle, SERVO_CENTER_ANGLE - 25, SERVO_CENTER_ANGLE + 25)))
                    ...
                    if avg_front_baseline < CORNER_APPROACH_TRIGGER_MM:
                        ...
                        if parallel_error is None:
                            ...
                            display_text = f"[CORNER] Phase 2: Approach (No Wall Data -> Straight) | Front: {avg_front_baseline:.0f}mm"
                            ...
                            display_text = f"[CORNER] Phase 2: Approach (Parallel Align) | Front: {avg_front_baseline:.0f}mm | Err: {parallel_error:.0f}mm"
                        ...
                        send_esp_packet(esp_ser, final_approach_servo, WALL_ALIGN_CREEP_SPEED)
            ...
            right_side_zone = [scan_data[a] for a in RIGHT_ZONE_ANGLES if a in scan_data and scan_data[a] > 0]

            left_side_zone = [scan_data[a] for a in LEFT_ZONE_ANGLES if a in scan_data and scan_data[a] > 0]


            # --- raw point-count condition: how many rays are under the trigger distance ---

            right_under = [d for d in right_side_zone if d < SIDE_TRIGGER_DISTANCE_MM]

            left_under = [d for d in left_side_zone if d < SIDE_TRIGGER_DISTANCE_MM]

            right_raw_trigger = len(right_under) >= SIDE_MIN_TRIGGER_POINTS

            left_raw_trigger = len(left_under) >= SIDE_MIN_TRIGGER_POINTS


            # --- ENTRY debounce: require SIDE_TRIGGER_CONFIRM_FRAMES consecutive raw triggers ---

            right_side_confirm_streak = right_side_confirm_streak + 1 if right_raw_trigger else 0

            left_side_confirm_streak = left_side_confirm_streak + 1 if left_raw_trigger else 0


            if not right_side_engaged and right_side_confirm_streak >= SIDE_TRIGGER_CONFIRM_FRAMES:

                right_side_engaged = True

                right_side_clear_streak = 0

            if not left_side_engaged and left_side_confirm_streak >= SIDE_TRIGGER_CONFIRM_FRAMES:
            ...
            # --- EXIT debounce: once engaged, require SIDE_RELEASE_CONFIRM_FRAMES consecutive clear frames ---

            if right_side_engaged:

                right_side_clear_streak = right_side_clear_streak + 1 if not right_raw_trigger else 0

                if right_side_clear_streak >= SIDE_RELEASE_CONFIRM_FRAMES:
            ...
            if left_side_engaged:
                ...
                left_side_clear_streak = left_side_clear_streak + 1 if not left_raw_trigger else 0

                if left_side_clear_streak >= SIDE_RELEASE_CONFIRM_FRAMES:
            ...
            if state_log_frame_counter % STATE_LOG_EVERY_N_FRAMES == 0:
                ...
                print(f"[SIDE DEBOUNCE] R: n={len(right_under)} streak={right_side_confirm_streak}/{SIDE_TRIGGER_CONFIRM_FRAMES} engaged={right_side_engaged} off={right_offset:.1f} | "

                      f"L: n={len(left_under)} streak={left_side_confirm_streak}/{SIDE_TRIGGER_CONFIRM_FRAMES} engaged={left_side_engaged} off={left_offset:.1f}")


            in_side_avoidance_cooldown = time.monotonic() < side_avoidance_cooldown_end_time  # NEW

            if (right_side_engaged or left_side_engaged) and not in_side_avoidance_cooldown:
                ...
                display_text = (
                    f"MODE: Side Avoid (Right, {right_min_dist:.0f}mm)" if right_offset >= left_offset and right_min_dist is not None
                    else f"MODE: Side Avoid (Left, {left_min_dist:.0f}mm)" if left_min_dist is not None
                    else "MODE: Side Avoid"
                )
            ...
            # ====================================================
                ...
                if in_side_avoidance_cooldown and (right_side_engaged or left_side_engaged) and state_log_frame_counter % STATE_LOG_EVERY_N_FRAMES == 0:
                    print(f"[SIDE AVOID] Suppressed - post-obstacle cooldown active ({side_avoidance_cooldown_end_time - time.monotonic():.2f}s left)")
                ...
                if obstacle_detected_this_frame:
                    ...
                    side_avoidance_cooldown_end_time = time.monotonic() + SIDE_AVOIDANCE_POST_OBSTACLE_COOLDOWN_SEC
