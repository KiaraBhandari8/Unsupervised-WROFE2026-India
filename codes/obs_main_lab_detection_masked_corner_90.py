...
# --- CORNER MANEUVER (simple: 35 cm front wall -> stop -> 90 deg pivot -> centre) ---
CORNER_TRIGGER_FRONT_MM = 350.0   # 35 cm from the front wall
CORNER_TURN_DEGREES = 90.0        # pivot target on the spot (gyro)
CORNER_PIVOT_SPEED = 0.5
CORNER_PIVOT_TIMEOUT_SEC = 2.5    # open-loop fallback if gyro stalls
CORNER_CENTRE_CREEP_SPEED = 0.3
CORNER_CENTRE_KP = 0.6
CORNER_CENTRE_TOL_MM = 40.0
CORNER_CENTRE_TIMEOUT_SEC = 3.0
CORNER_BRAKE_DELAY_SEC = 0.25
CORNER_COOLDOWN_SEC = 6.0
...
def _corner_front_mm(scan_data):
    if not scan_data:
        return None
    pts = [d for a, d in scan_data.items() if -10 <= a <= 10 and d > 0]
    return float(np.median(pts)) if pts else None


def _side_avg_mm(scan_data, lo, hi):
    if not scan_data:
        return None
...
class RobotState:
    ...
    CORNER_CENTRE = "CORNER_CENTRE"

current_robot_state = RobotState.INITIALIZING

# Corner cooldown register (module level, persists across loops)
...
def robot_control_loop(shutdown_event):
    ...
    global CLOCKWISE_WALL_FOLLOWING
    global corner_cooldown_end_time
    ...
    try:
        ...
        while not shutdown_event.is_set():
            ...
            # ====================================================
            # CORNER LOGIC (simple):
            #   35 cm front wall -> STOP -> gyro 90 deg pivot on
            #   the spot -> centre between the walls -> drive on.
            # ====================================================
            avg_left_side = _side_avg_mm(scan_data, -90, -40)
            avg_right_side = _side_avg_mm(scan_data, 40, 90)
            avg_front_dist = _corner_front_mm(scan_data)
            in_corner_cooldown = time.monotonic() < corner_cooldown_end_time

            # --- Active corner state machine ---
            if current_robot_state in (RobotState.CORNER_PIVOT, RobotState.CORNER_CENTRE):
                if current_robot_state == RobotState.CORNER_PIVOT:
                    yaw_now = current_yaw if current_yaw is not None else 0.0
                    yaw_delta = yaw_now - baseline_start_yaw
                    if turn_direction == "RIGHT":
                        pivot_done = yaw_delta <= -CORNER_TURN_DEGREES
                    else:
                        pivot_done = yaw_delta >= CORNER_TURN_DEGREES
                    pivot_timeout = (time.monotonic() - pivot_phase_start_time) >= CORNER_PIVOT_TIMEOUT_SEC
                    display_text = f"[CORNER] Pivot {yaw_delta:+.1f}deg / {CORNER_TURN_DEGREES:.0f}deg ({turn_direction})"
                    print(f"[CORNER] Pivot {yaw_delta:+.1f}deg / {CORNER_TURN_DEGREES:.0f}deg ({turn_direction})")
                    if pivot_done or pivot_timeout:
                        if pivot_timeout and not pivot_done:
                            print(f"[CORNER] Pivot timeout at {yaw_delta:+.1f}deg.")
                        stop_robot()
                        ...
                        if ser and ESP32_OK:
                            ...
                            ser.flush()
                            time.sleep(0.1)
                        centre_start_time = time.monotonic()
                        current_robot_state = RobotState.CORNER_CENTRE
                    else:
                        hard = SERVO_HARD_RIGHT if turn_direction == "RIGHT" else SERVO_HARD_LEFT
                        cmd(hard, CORNER_PIVOT_SPEED)
                else:  # CORNER_CENTRE
                    left_m = avg_left_side
                    right_m = avg_right_side
                    centre_done = False
                    if left_m is not None and right_m is not None:
                        centre_err = right_m - left_m
                        display_text = f"[CORNER] Centre err {centre_err:+.0f}mm (L {left_m:.0f} R {right_m:.0f})"
                        print(f"[CORNER] Centre err {centre_err:+.0f}mm (L {left_m:.0f} R {right_m:.0f})")
                        if abs(centre_err) <= CORNER_CENTRE_TOL_MM:
                            centre_done = True
                        else:
                            steer = int(round(np.clip(
                                SERVO_CENTER_ANGLE + CORNER_CENTRE_KP * centre_err,
                                SERVO_CENTER_ANGLE - 25, SERVO_CENTER_ANGLE + 25)))
                            cmd(steer, CORNER_CENTRE_CREEP_SPEED)
                    else:
                        centre_done = (time.monotonic() - centre_start_time) >= 1.0
                        display_text = "[CORNER] Centre (no walls)"
                        if not centre_done:
                            cmd(SERVO_CENTER_ANGLE, CORNER_CENTRE_CREEP_SPEED)
                    centre_timeout = (time.monotonic() - centre_start_time) >= CORNER_CENTRE_TIMEOUT_SEC
                    if centre_done or centre_timeout:
                        if centre_timeout and not centre_done:
                            print("[CORNER] Centre timeout.")
                        stop_robot()
                        time.sleep(0.2)
                        if ser and ESP32_OK:
                            ser.write(b"RST_YAW\n")
                        ...
                        turn_direction = None
                        print(f"[CORNER] Centred. Turns completed: {turn_counter}")
                if DEBUG_UI_OVERLAYS:
                    processed_frame = frame_bgr.copy()
                    cv2.putText(processed_frame, display_text, (20, 40),
            ...
            # --- Corner detection: front wall at 35 cm ---
            if (not in_corner_cooldown
                    and avg_front_dist is not None
                    and avg_front_dist <= CORNER_TRIGGER_FRONT_MM):
                print(f"\n[CORNER] Front wall {avg_front_dist:.0f}mm <= {CORNER_TRIGGER_FRONT_MM:.0f}mm. Stopping...")
                stop_robot()
                time.sleep(CORNER_BRAKE_DELAY_SEC)
                if ser and ESP32_OK:
                    ser.write(b"RST_YAW\n")
                    ser.flush()
                    time.sleep(0.1)
                if (avg_left_side is not None and avg_right_side is not None
                        and avg_left_side <= avg_right_side):
                    turn_direction = "RIGHT"
                    CLOCKWISE_WALL_FOLLOWING = True
                else:
                    turn_direction = "LEFT"
                    CLOCKWISE_WALL_FOLLOWING = False
                baseline_start_yaw = current_yaw if current_yaw is not None else 0.0
                pivot_phase_start_time = time.monotonic()
                hard = SERVO_HARD_RIGHT if turn_direction == "RIGHT" else SERVO_HARD_LEFT
                cmd(hard, 0)
                time.sleep(0.15)
                current_robot_state = RobotState.CORNER_PIVOT
                print(f"[CORNER] Turning {turn_direction} on the spot (hard servo {hard})")
                cmd(hard, CORNER_PIVOT_SPEED)
                pace_remaining = (1.0 / CONTROL_LOOP_MAX_HZ) - (time.monotonic() - loop_start_time)
                if pace_remaining > 0:
                    time.sleep(pace_remaining)
...
@app.route("/camera")
def camera_feed():
    return render_template("obs_camera.html")


@app.route("/get_robot")
def get_robot():
    remaining = max(0.0, corner_cooldown_end_time - time.monotonic())
    with yaw_lock:
        yaw = latest_yaw
    return jsonify({
        ...
        "corner_cooldown": round(remaining, 1),
    })
