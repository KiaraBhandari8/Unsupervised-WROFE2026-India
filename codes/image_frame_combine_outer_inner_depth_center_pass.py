...
# --- NEW: GAP-CENTERING (thread the middle of the open lane beside the obstacle) ---
GAP_CENTER_BIAS = 0.5   # 0.5 = exact middle of the gap between the obstacle's edge and the ROI boundary.
                         # <0.5 = hug closer to the obstacle (tighter squeeze, more track margin on the far side).
                         # >0.5 = hug closer to the ROI/track edge (safer clearance from the obstacle, less margin from the wall).
                         # Tune this in small steps (0.45 / 0.55) once the basic behavior looks right.
...
def process_frame_for_steering(frame, use_outer_roi_and_bottom_point=False):
    ...
    if chosen_contour is not None:
        ...
        if logic_label == "red_obstacle":
            ...
            # Pass on the LEFT of a red obstacle. Thread the middle of the corridor
            # between the obstacle's own left edge and the ROI's left boundary,
            # instead of always steering all the way out to that boundary.
            obstacle_near_edge = x  # obstacle's left edge (bbox x)
            gap_target_x = int(obstacle_near_edge + GAP_CENTER_BIAS * (red_target_x - obstacle_near_edge))
            gap_target_x = int(np.clip(gap_target_x, 0, w - 1))
            error = gap_target_x - pX
            steering_angle = KP_STEERING * error * depth_factor
            obstacle_target_x = gap_target_x
        else: # Green obstacle
            output_mask = binary_green
            # Pass on the RIGHT of a green obstacle -- mirrored gap-centering.
            obstacle_near_edge = x + wc  # obstacle's right edge
            gap_target_x = int(obstacle_near_edge + GAP_CENTER_BIAS * (green_target_x - obstacle_near_edge))
            gap_target_x = int(np.clip(gap_target_x, 0, w - 1))
            error = gap_target_x - pX
            steering_angle = KP_STEERING * error * depth_factor
            obstacle_target_x = gap_target_x
