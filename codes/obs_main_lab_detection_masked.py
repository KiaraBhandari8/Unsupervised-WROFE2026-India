...
STEERING_GAIN = 0.1
ROBOT_MANEUVER_SPEED = 0.8 #0.8
ROBOT_CRUISE_SPEED =  0.85  #0.85
ROBOT_SPEED_MAX = 0.9
...
LEFT_DISTANCE_ARC_THRESHOLD_MM = 400.0
CORNER_ARC_TRIGGER_FRONT_MM = 280.0
...
def robot_control_loop(shutdown_event):
    ...
    try:
        ...
        # Corner maneuver state variables
        turn_count = 0
        ...
        corner_outer_wall_distance_mm = 0.0
        ...
        while not shutdown_event.is_set():
            ...
            # Compute side wall averages for corner detection
            avg_left = 2000.0
            avg_right = 2000.0
            if scan_data:
            ...
            # Compute compensated front distance for corner detection
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ARC_ACTIVE_PIVOT, RobotState.CORNER_POST_PIVOT_ALIGN, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    ...
                    print(f"[{current_robot_state}] [CORNER EXECUTION] Phase 2: Approaching wall | Front: {avg_front_baseline:.1f}mm / {approach_trigger_distance:.1f}mm | Yaw: {yaw_for_corner:+.1f}deg | ArcMode: {use_reverse_arc}")
                    ...
                    if avg_front_baseline < approach_trigger_distance:
                        ...
                        if use_reverse_arc:
                            ...
                            print(f"[{current_robot_state}] [CORNER EXECUTION] Entering reverse arc pivot: steer={arc_steer_angle}deg speed=-{CORNER_ARC_PIVOT_SPEED}")
                            ...
                            print(f"[{current_robot_state}] [CORNER EXECUTION] Locking wheels to extreme pivot angle: {final_servo}deg")
                            cmd(final_servo, 0)
                            time.sleep(0.15)
                    ...
                    display_text = f"[CORNER] Phase 3: Gyro Pivot | Yaw: {yaw_delta_signed:+.1f}deg / target {target_degrees}deg ({turn_direction})"
                    ...
                    if pivot_complete or pivot_timed_out:
                        ...
                        print(f"[{current_robot_state}] [CORNER EXECUTION] Pivot complete ({yaw_delta_signed:+.1f}deg). Hard braking chassis...")
                        cmd(SERVO_CENTER_ANGLE, 0)
                        time.sleep(CORNER_BRAKE_DELAY)

                        print(f"[{current_robot_state}] [CORNER EXECUTION] Resetting chassis orientation before post-pivot align...")
                        ser.write(b"RST_YAW\n")
                        ser.flush()
                        time.sleep(0.1)
                        yaw_for_corner = 0.0
                    ...
                    if arc_complete or arc_timed_out or rear_tof_hit:
                        ...
                        if rear_tof_hit:
                            print(f"[{current_robot_state}] [ARC PIVOT] Rear ToF limit reached ({rear_distance:.0f}mm) before yaw target. Stopping arc early.")
                    ...
                    if is_aligned or hard_timeout or no_wall_timeout:
                        ...
                        print(f"[{current_robot_state}] [POST-PIVOT ALIGN] Exit condition reached ({reason}). Outer wall: {corner_outer_wall_distance_mm:.0f}mm")
                        cmd(SERVO_CENTER_ANGLE, 0)
                        time.sleep(CORNER_BRAKE_DELAY)

                        if corner_outer_wall_distance_mm < 60.0:
                            print(f"[{current_robot_state}] [POST-PIVOT ALIGN] Outer wall too close ({corner_outer_wall_distance_mm:.0f}mm < 60mm). Skipping backward phase.")
                            ser.write(b"RST_YAW\n")
                            ser.flush()
                            time.sleep(0.1)
                            ...
                            print(f"[{current_robot_state}] [POST-PIVOT ALIGN] Outer wall has clearance ({corner_outer_wall_distance_mm:.0f}mm >= 60mm). Entering backward phase.")
                        ...
                        if parallel_error is None:
                            ...
                            steer_offset = -0.5 * normalized_error
                            target_servo_angle = SERVO_CENTER_ANGLE - steer_offset
                    ...
                    if tof_reached or hard_timeout_backward:
                        if hard_timeout_backward and not tof_reached:
                            print(f"[{current_robot_state}] [CORNER EXECUTION] WARNING: Backward safety timeout hit before ToF target (rear={rear_distance}).")
                        else:
                            print(f"[{current_robot_state}] [CORNER EXECUTION] ToF target reached (rear={rear_distance:.0f}mm).")
                        cmd(SERVO_CENTER_ANGLE, 0)
                        time.sleep(0.3)
            ...
            # ====================================================
            # PRIORITY LEVEL 2.5: RADAR FRONTAL TRIPWIRE CHECK (CORNER DETECTION)
            # ====================================================
            ...
            if (lidar_scanner
                ...
                and ((avg_left < 950.0 and avg_right > 1600.0)
                     or (avg_right < 900.0 and avg_left > 1600.0))):
            ...
            if is_corner_signature:
                ...
                if avg_left < avg_right:
                    ...
                    corner_outer_wall_distance_mm = avg_left
                    ...
                    turn_direction = "LEFT"
                    corner_outer_wall_distance_mm = avg_right
                    use_reverse_arc = False
                    if turn_count == 0:
                        CLOCKWISE_WALL_FOLLOWING = False
            ...
            # PRIORITY 0: OBSTACLE AVOIDANCE (always highest)
            if avoid_engaged:
            ...
            # PRIORITY 1: IMMINENT COLLISION (front head-on)
            ...
            # PRIORITY 2.5: IMMINENT SIDE COLLISION (tighter than normal side alerts)
            elif scan_data and check_imminent_right_side(scan_data):
                current_robot_state = RobotState.LIDAR_SIDE_AVOIDANCE
                target_servo_angle = SERVO_CENTER_ANGLE - IMMINENT_SIDE_STEER_MAGNITUDE
                robot_speed_current = ROBOT_MANEUVER_SPEED
                display_text = "MODE: IMMINENT | Right Side!"
                if time.time() - print_timer >= 0.5:
                    print_timer = time.time()
                    print("IMMINENT Side: RIGHT")
            elif scan_data and check_imminent_left_side(scan_data):
                current_robot_state = RobotState.LIDAR_SIDE_AVOIDANCE
                target_servo_angle = SERVO_CENTER_ANGLE + IMMINENT_SIDE_STEER_MAGNITUDE
                robot_speed_current = ROBOT_MANEUVER_SPEED
                display_text = "MODE: IMMINENT | Left Side!"
                if time.time() - print_timer >= 0.5:
                    print_timer = time.time()
                    print("IMMINENT Side: LEFT")

            # PRIORITY 3: LIDAR WALL FOLLOWING
            elif lidar_scanner and lidar_pid:
                # Post-obstacle wall alignment: after finishing a pillar pass,
                # realign with the wall using LiDAR parallel error before resuming
