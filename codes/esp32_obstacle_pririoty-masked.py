...
try: 
    from image_frame_combine_outer_inner_depth_masked import (
        process_frame_for_steering,
        get_presets, update_preset, save_presets, load_presets,
        segment_for_stream, segment_track_for_stream,
        STREAM_COLORS,
    )
...
TURN_TARGET_RIGHT_DEGREES = 60.0    # TUNED (was 80.0)
TURN_TARGET_LEFT_DEGREES = 60.0     # TUNED (was 80.0)
...
# ====================================================
# NEW: WEB TUNING STREAMS (ported from V2's per-colour / track debug feeds)
# Each of these grabs the LATEST raw camera frame (not the annotated control
# frame) and re-segments it live using whatever is currently in COLOR_PRESETS
# -- so tuning a slider on the /tuning page is visible within one frame, with
# no restart needed. Deliberately capped/downscaled so multiple simultaneous
# MJPEG viewers don't overload the Pi -- these are debug/tuning feeds, not the
# primary driving stream.
# ====================================================
TUNING_STREAM_WIDTH = 480
TUNING_STREAM_MAX_FPS = 10
TUNING_STREAM_JPEG_QUALITY = 65


def _get_latest_bgr_for_tuning():
    with camera_frame_lock:
...
def _downscale_for_tuning(img):
    if img.shape[1] > TUNING_STREAM_WIDTH:
        new_h = int(img.shape[0] * TUNING_STREAM_WIDTH / img.shape[1])
        img = cv2.resize(img, (TUNING_STREAM_WIDTH, new_h), interpolation=cv2.INTER_AREA)
    return img


def generate_color_stream(color):
    last_emit = 0.0
    while not global_shutdown_event.is_set():
        if not STREAM_VIDEO:
            time.sleep(0.5)
            continue
        min_interval = 1.0 / max(1, TUNING_STREAM_MAX_FPS)
        wait = min_interval - (time.monotonic() - last_emit)
        if wait > 0:
            time.sleep(wait)
        last_emit = time.monotonic()

        frame_bgr = _get_latest_bgr_for_tuning()
        if frame_bgr is None:
            time.sleep(0.05)
            continue

        if color == "track":
            img = segment_track_for_stream(frame_bgr)
        else:
            img = segment_for_stream(frame_bgr, color)

        img = _downscale_for_tuning(img)
        encode_params = [int(cv2.IMWRITE_JPEG_QUALITY), TUNING_STREAM_JPEG_QUALITY]
...
def index():
    return (
        "<h3>WRO 2026 Live Camera Server Active</h3>"
        "<img src='/video_feed' width='100%'/><br><br>"
        "<a href='/tuning'>Open LAB colour tuning page</a>"
    )
...
# ====================================================
# NEW: LAB PRESET TUNING ENDPOINTS (ported from V2's /get_presets, /update,
# /save, /load) -- lets you retune red/green/white LAB ranges from the web
# page below without touching code or restarting the robot.
# ====================================================
@app.route("/video/<color>")
def video_color_feed(color):
    if color not in STREAM_COLORS and color != "track":
        return "Unknown stream", 404
    return Response(generate_color_stream(color), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route("/get_presets")
def route_get_presets():
    return jsonify(get_presets())


@app.route("/update_preset", methods=["POST"])
def route_update_preset():
    data = request.json or {}
    color = data.get("mode")
    values = data.get("values", {})
    ok = update_preset(color, values) if color else False
    return jsonify({"ok": ok})


@app.route("/save_presets")
def route_save_presets():
    ok = save_presets()
    return jsonify({"saved": ok})


@app.route("/load_presets")
def route_load_presets():
    loaded = load_presets()
    return jsonify({"loaded": True, "presets": loaded})
...
<html>
<head>
<title>LAB Colour Tuning</title>
<style>
  body { font-family: sans-serif; background:#111; color:#eee; margin:0; padding:16px; }
  h2 { margin-top:0; }
  .row { display:flex; flex-wrap:wrap; gap:20px; }
  .panel { background:#1c1c1c; border-radius:8px; padding:12px 16px; width:300px; }
  .panel h3 { margin:4px 0 10px 0; text-transform:capitalize; }
  .panel img { width:100%; border-radius:6px; margin-bottom:10px; background:#000; }
  ...
  button { flex:1; padding:8px; border:none; border-radius:6px; cursor:pointer;
           background:#2d6cdf; color:#fff; font-weight:bold; }
  button.secondary { background:#444; }
  #status { margin:10px 0; color:#8f8; min-height:1.2em; }
</style>
</head>
<body>
<h2>LAB Colour Tuning</h2>
<div id="status"></div>
<div class="row">
  <div class="panel">
  ...
  <div class="panel">
    <h3>Drivable track (false-positive filter)</h3>
    <img src="/video/track">
  </div>
</div>
<div class="row" id="colorPanels"></div>

<script>
const COLORS = {{ colors_json|safe }};
const container = document.getElementById('colorPanels');
const status = document.getElementById('status');

function fieldRow(color, key, label) {
  return `<label>${label} <span class="valtxt" id="${color}_${key}_v"></span>
    <input type="range" min="0" max="255" id="${color}_${key}"
      oninput="onSlide('${color}','${key}')"></label>`;
}
...
COLORS.forEach(color => {
  ...
  panel.innerHTML = `
    <h3>${color}</h3>
    <img src="/video/${color}">
    ${fieldRow(color,'l_min','L min')}
    ${fieldRow(color,'l_max','L max')}
    ${fieldRow(color,'a_min','A min')}
    ${fieldRow(color,'a_max','A max')}
    ${fieldRow(color,'b_min','B min')}
    ${fieldRow(color,'b_max','B max')}
  `;
  container.appendChild(panel);
});

function onSlide(color, key) {
  const v = parseInt(document.getElementById(`${color}_${key}`).value, 10);
  document.getElementById(`${color}_${key}_v`).textContent = v;
  fetch('/update_preset', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({mode: color, values: {[key]: v}})
  });
}

function loadPresetsIntoUI() {
  fetch('/get_presets').then(r => r.json()).then(presets => {
    COLORS.forEach(color => {
      const p = presets[color];
      if (!p) return;
      ['l_min','l_max','a_min','a_max','b_min','b_max'].forEach(key => {
        const el = document.getElementById(`${color}_${key}`);
        if (el && p[key] !== undefined) {
          el.value = p[key];
          document.getElementById(`${color}_${key}_v`).textContent = p[key];
        }
      });
    });
  });
...
function saveToDisk() {
  fetch('/save_presets').then(r => r.json()).then(d => {
    status.textContent = d.saved ? 'Saved presets to disk.' : 'Save FAILED (check server log).';
  });
}

function loadFromDisk() {
  fetch('/load_presets').then(r => r.json()).then(() => {
    status.textContent = 'Reloaded presets from disk.';
    loadPresetsIntoUI();
  });
}

setTimeout(loadPresetsIntoUI, 300);  // wait for panels to render first
</script>

<div class="btnrow" style="max-width:620px;">
  <button onclick="saveToDisk()">Save presets to disk</button>
  <button class="secondary" onclick="loadFromDisk()">Reload presets from disk</button>
</div>
...
@app.route("/tuning")
def tuning_page():
    return render_template_string(_TUNING_PAGE_HTML, colors_json=json.dumps(STREAM_COLORS))
