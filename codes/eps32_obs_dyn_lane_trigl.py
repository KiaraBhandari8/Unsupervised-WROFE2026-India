...
# ported into lidar_steering4sept.py (they previously only existed in lidar_steering_new.py).
# If they aren't there yet, copy them over first or this import will fail.
try: 
...
# --- NEW: CORNER DETECTION PARAMETERS ---
...
CORNER_DETECTION_COOLDOWN_SEC = 8.0 # Safety guard so we don't re-trigger immediately after a turn (added; remove if unwanted)

# --- NEW: LANE DETECTION PARAMETER ---
LANE_SIDE_THRESHOLD_MM = 600.0      # > this on the hugged side => lane 1 (needs backward); else lane 2/3 (skip backward)

# --- NEW: BACKWARD PHASE PARAMETERS (LiDAR rear-angle substitute for ToF) ---
...
ROBOT_CRUISE_SPEED = 160 #160      # Operational forward driving speed sent to ESP32 (0-255)
ROBOT_MANEUVER_SPEED = 150 # 150    # Slowdown velocity used across complex evasion arcs / backward
...
SERVO_HARD_LEFT = 0
CORNER_PIVOT_SAFETY_TIMEOUT = 3.0   # Safety guard so a stuck/noisy gyro can't spin forever (added; not in your original spec)
...
class RobotState:
    ...
    CORNER_DETECT_STOP = "CORNER_DETECT_STOP"               # NEW: 0.5s stop + average to confirm corner + decide direction/lane
    CORNER_APPROACH_ALIGN = "CORNER_APPROACH_ALIGN"          # NEW: drive to the wall using parallel follow until front <= 550mm
    CORNER_ACTIVE_PIVOT = "CORNER_ACTIVE_PIVOT"              # gyro-yaw in-place pivot (same mechanic as before)
    CORNER_ALIGN_BACKWARD = "CORNER_ALIGN_BACKWARD"          # NEW: conditional reverse, only when lane_needs_backward is True
...
# --- NEW: SHARED LIDAR SECTOR HELPERS ---
...
def compute_parallel_steering(scan_data, side, pid):
    """Triangle-method parallel wall following (ported logic/sign convention from the
    other file's WALL_ALIGN_CORRECTION). Returns (target_servo_angle, display_text)."""
    parallel_error = get_wall_parallel_error(scan_data, side)
    if parallel_error is None:
        # No wall detected on the followed side -- no gyro fallback per your spec,
...
def robot_control_loop():
    ...
    # remains anywhere in this file (per your instructions). Gyro/yaw is only used
    # during the in-place pivot to know when the turn is complete.
    ...
    try:
        while not global_shutdown_event.is_set():
            ...
            if current_robot_state in [RobotState.CORNER_DETECT_STOP, RobotState.CORNER_APPROACH_ALIGN,
                ...
                # --- PHASE 3: In-place gyro pivot (same mechanic as your original code) ---
            ...
            # ====================================================
            # PRIORITY LEVEL 2: CORNER SIGNATURE TRIPWIRE (new thresholds)
            # ====================================================
