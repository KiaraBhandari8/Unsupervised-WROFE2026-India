...
ROBOT_CRUISE_SPEED = 255      # Operational forward driving speed sent to ESP32 (0-255)
ROBOT_MANEUVER_SPEED = 180    # Slowdown velocity used across complex evasion arcs
...
def robot_control_loop():
    ...
    try:
        while not global_shutdown_event.is_set():
            ...
            # Hard front-stop tripwire: force corner entry when wall is critically close
            # even if side asymmetry doesn't match a typical corner signature
            if (not is_corner_signature
                and not in_cooldown
                and avg_front_baseline < FRONT_TURN_TRIGGER_MM):
                print(f"[SAFETY TRIPWIRE] Front wall critically close ({avg_front_baseline:.0f}mm < {FRONT_TURN_TRIGGER_MM}mm). Forcing corner entry.")
                is_corner_signature = True
