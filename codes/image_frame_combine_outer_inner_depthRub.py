...
def process_frame_for_steering(frame, use_outer_roi_and_bottom_point=False):
    ...
    NOTE: the 5th return value used to be a hardcoded 0. It now returns the actual
    depth_factor computed below (0.0 when no obstacle / far away, up to
    DEPTH_IMPORTANCE_FACTOR when the obstacle is at the very bottom of the frame /
    closest). Callers can use this to scale any proximity-dependent correction
    (e.g. a clearance offset) instead of applying a flat, distance-independent value.
    """
    if frame is None:
    ...
    depth_factor = 0.0  # NEW default: 0.0 when there's no obstacle at all (was 1.0, but "no
                         # effect" only makes sense when steering_angle is also 0; returning
                         # 0.0 here makes it a clean, honest "no proximity urgency" signal for
                         # callers that scale a correction by this value).
    if chosen_contour is not None:
        ...
        # This scales linearly from 0 (far) up to DEPTH_IMPORTANCE_FACTOR (closest).
