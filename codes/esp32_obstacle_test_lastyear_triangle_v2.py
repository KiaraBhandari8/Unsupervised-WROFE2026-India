...
LIDAR_RIGHT_SIDE_DISTANCE_MM = 250  # 25cm side distance panic limit (widened to prevent inner wall contact)
LIDAR_LEFT_SIDE_DISTANCE_MM = 250   # 25cm side distance panic limit (widened to prevent inner wall contact)
LIDAR_SIDE_STEER_MAGNITUDE = 25     # Stronger steering shift away from side walls
...
FRONT_TURN_TRIGGER_MM = 350.0  # Wider front emergency stop boundary (35cm)
FRONT_SCAN_ANGLE_DEG = 20      # Wider front scan cone (+/- 20°) for better detection
FRONT_EMERGENCY_STOP_MM = 350.0  # Universal front wall stop (catches walls the corner signature misses)
...
CORNER_APPROACH_BRAKE_DISTANCE_MM = 900.0  # Brake much earlier (was 600, now 900)
CORNER_PIVOT_SPEED = 100
CORNER_PIVOT_SAFETY_TIMEOUT = 3.0
CORNER_BACKWARD_DURATION = 3.0  # Reverse longer to clear inner corner (was 2.0, now 3.0)
        ...
        while not global_shutdown_event.is_set():
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    ...
                    print(f"[{current_robot_state}] [CORNER EXECUTION] Phase 2: Approaching wall | Front: {avg_front_baseline:.1f}mm / {CORNER_APPROACH_BRAKE_DISTANCE_MM:.1f}mm | Yaw: {current_yaw:+.1f}°")

                    if avg_front_baseline < CORNER_APPROACH_BRAKE_DISTANCE_MM:
                        ...
                        # Use maneuver speed on approach to avoid ramming the wall
            ...
            # Universal front wall emergency stop — catches any wall ahead, not just corner signatures
            is_front_emergency = False
            if (not in_cooldown
                and current_robot_state not in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]
                and avg_front_baseline <= FRONT_EMERGENCY_STOP_MM):
                # If there's a wall directly ahead and we're not already cornering, treat as a corner
                is_front_emergency = True

            is_corner_signature = False
            if (not in_cooldown
                and avg_front_baseline <= 1300.0  # Wider front detection range (was 1000)
                and ((avg_left < 1200.0 and avg_right > 1400.0)  # Relaxed side thresholds
                     or (avg_right < 1200.0 and avg_left > 1400.0))):
                is_corner_signature = True

            if is_front_emergency or is_corner_signature:
                reason_tag = "FRONT WALL" if is_front_emergency else "signature"
                print(f"\n[CORNER INTERSECTION] Front Wall at: {avg_front_baseline:.1f}mm. Stopping chassis... ({reason_tag})")
                send_esp_packet(esp_ser, SERVO_CENTER_ANGLE, 0)
                time.sleep(1.0)
            ...
            right_side_panic = [scan_data[a] for a in range(30, 85) if a in scan_data and 0 < scan_data[a] < LIDAR_RIGHT_SIDE_DISTANCE_MM]
            left_side_panic = [scan_data[a] for a in range(-84, -29) if a in scan_data and 0 < scan_data[a] < LIDAR_LEFT_SIDE_DISTANCE_MM]
