...
# Steering clip widened to ±40° (55–135)
# Wall-loss threshold fixed (350 → 750mm) — this was actually a logic bug, not just a tuning value
# Gyro-based pivot       
...
CORNER_BACKWARD_DURATION = 3.0 # Seconds to back away from the corner wall prev 1.5
        ...
        while not global_shutdown_event.is_set():
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    ...
                    print(f"[CORNER EXECUTION] Phase 2: Approaching wall | Front: {avg_front_baseline:.1f}mm / 450.0mm")
                ...
                # PHASE 3: Gyro-Based Pivot (rotate until measured yaw delta hits target,
                # with a hard time cap as a safety fallback if the gyro stalls)
                elif current_robot_state == RobotState.CORNER_ACTIVE_PIVOT:
                    elapsed_pivot = time.monotonic() - pivot_phase_start_time
                    yaw_delta = abs(current_yaw - baseline_start_yaw)
                    display_text = f"[CORNER] Phase 3: Gyro Pivot | Yaw: {yaw_delta:.1f}° / {TURN_TARGET_DEGREES}°"
                    
                    pivot_complete = yaw_delta >= TURN_TARGET_DEGREES
                    ...
                    if pivot_complete or pivot_timed_out:
                        if pivot_timed_out and not pivot_complete:
                            print(f"[CORNER EXECUTION] WARNING: Pivot safety timeout hit before target yaw reached (only {yaw_delta:.1f}° / {TURN_TARGET_DEGREES}°). Check gyro data.")
                        print(f"[CORNER EXECUTION] Pivot complete ({yaw_delta:.1f}°). Hard braking chassis...")
                        send_esp_packet(esp_ser, SERVO_CENTER_ANGLE, 0)
            ...
            if (avg_front_baseline <= 1000.0) and ((avg_left < 900.0 and avg_right > 1900.0) or (avg_right < 900.0 and avg_left > 1400.0)):
            ...
            # ====================================================
            elif logic_label in ["red_obstacle", "obstacle"]:
