...
CORNER_PIVOT_DURATION = 1.15   # Seconds needed to execute a perfect 90-degree turn
            ...
            if current_robot_state in [RobotState.CORNER_APPROACH_WALL, RobotState.CORNER_ACTIVE_PIVOT, RobotState.CORNER_ALIGN_BACKWARD]:
                ...
                if current_robot_state == RobotState.CORNER_APPROACH_WALL:
                    ...
                    if avg_front_baseline < 450.0:
                        ...
                        # Initialize high-resolution runtime stopwatch
                ...
                # PHASE 3: Deterministic Open-Loop Time Pivot (Gyro Ignored)
                elif current_robot_state == RobotState.CORNER_ACTIVE_PIVOT:
                    elapsed_pivot = time.monotonic() - pivot_phase_start_time
                    display_text = f"[CORNER] Phase 3: Timed Pivot | {elapsed_pivot:.2f}s / {CORNER_PIVOT_DURATION}s"
                    
                    if elapsed_pivot >= CORNER_PIVOT_DURATION:
                        print(f"[CORNER EXECUTION] Pivot time window reached. Hard braking chassis...")
                        send_esp_packet(esp_ser, SERVO_CENTER_ANGLE, 0)
