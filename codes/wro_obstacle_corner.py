...
# source.
CAMERA_HALF_FOV_DEG = 50.0       # ~half of a typical wide-angle lens' horizontal FOV
STEERING_GAIN_RED = 0.14         # servo degrees per pseudo-degree of vision offset (red)
STEERING_GAIN_GREEN = 0.10       # servo degrees per pseudo-degree of vision offset (green)
RED_CLEARANCE_OFFSET = 8         # extra fixed rightward bias while avoiding red (ESP32 value)
...
def vision_pillar_avoidance(dets, lab_w):
    ...
    # scaled to a pseudo-degree "vision angle" over the camera's horizontal
    # FOV -- the same role the ESP32 firmware's outer/inner-ROI vision module
    # played, just derived from our own LAB detection instead.
...
CORNER_SIGNATURE_FRONT_MM = 1000.0  # front must be closing to at least this range
CORNER_SIGNATURE_TIGHT_MM = 900.0   # the wall-side reading must be tighter than this
CORNER_SIGNATURE_OPEN_MM = 1400.0   # the open-side reading must be at least this
CORNER_APPROACH_STOP_MM = 450.0     # keep creeping toward the wall until this close
CORNER_PIVOT_DURATION_SEC = 1.15    # time to hold the hard-locked pivot
...
def manage_corner_turn(scan_data, current_yaw=None, lidar_pid=None):
    ...
    if current_robot_state == RobotState.CORNER_ALIGN_BACKWARD:
        ...
        if elapsed >= CORNER_BACKWARD_DURATION_SEC:
            ...
            return True, SERVO_CENTER_ANGLE, 0, f"[CORNER] Turn #{turn_counter} complete"
